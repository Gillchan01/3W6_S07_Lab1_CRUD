...
namespace ZombieParty.Controllers
{
    public class ZombieController : Controller
    {
        ...
        {
            List<Zombie> zombiesList = _baseDonnees.Zombies.OrderBy(z => z.Name)
                .Include(z => z.ZombieType).ToList();

            return View(zombiesList);
        }
        public IActionResult StrongestZombies()
        {
            List<Zombie> zombiesList = _baseDonnees.Zombies.Where(z => z.Point >= 8).OrderBy(z => z.Name)
                .Include(z => z.ZombieType).ToList();
